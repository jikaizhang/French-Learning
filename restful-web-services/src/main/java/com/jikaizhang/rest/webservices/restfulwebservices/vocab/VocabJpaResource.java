...
@RestController
public class VocabJpaResource {
	
	@Autowired
	private VocabHardcodedService vocabService;
	
	@Autowired
	private VocabJpaRepository vocabJpaRepository;
	
	@GetMapping("/jpa/users/{username}/vocabs")
	public List<Vocab> getAllVocabs(@PathVariable String username) {
		return vocabJpaRepository.findByUsername(username);
//		return vocabService.findAll();
	}
	
	@GetMapping("/jpa/users/{username}/vocabs/{id}")
	public Vocab getVocab(@PathVariable String username, @PathVariable long id) {
		return vocabJpaRepository.findById(id).get();
//		return vocabService.findById(id);
	}
	
	@DeleteMapping("/jpa/users/{username}/vocabs/{id}")
	public ResponseEntity<Void> deleteVocab(
			@PathVariable String username, @PathVariable long id) {
//		Vocab vocab = vocabService.deleteById(id);
		vocabJpaRepository.deleteById(id);
		
		return ResponseEntity.noContent().build();
//		return ResponseEntity.notFound().build();
	}
	
	@PutMapping(path="/jpa/users/{username}/vocabs/{id}")
...
//		Vocab vocabUpdated = vocabService.save(vocab);
		Vocab vocabUpdated = vocabJpaRepository.save(vocab);
		
		return new ResponseEntity<Vocab>(vocab, HttpStatus.OK);
	}
	
	@PostMapping(path="/jpa/users/{username}/vocabs")
	public ResponseEntity<Void> createVocab(
			@PathVariable String username, @RequestBody Vocab vocab) {
//		Vocab vocabCreated = vocabService.save(vocab);
		vocab.setUsername(username);
		Vocab vocabCreated = vocabJpaRepository.save(vocab);
